...
| M14 网络源测速与回环中继 | 2026-09-11 按用户需求与 `doc/契约补充-v1-增补2.md`（C16~C19）立项；T14.1（契约与设计）**已完成**；T14.2（测速排序）/ T14.3（`internal/relay` 回环中继）/ T14.4（uv 接线与锁规划）/ T14.5（CLI 预热、字节进度、supervise 常驻）/ T14.6（protocol 字段与 `network.probe`）/ T14.7（组件矩阵与黑盒）🚧 |
