...
func TestLayout_FixedPathsMatchSpecification(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "VenvConfigFile", got: layout.VenvConfigFile(), want: filepath.Join(root, "runtime", "environment", "venv", "pyvenv.cfg")},
...
func fixedPaths(layout *config.Layout) []fixedPath {
	return []fixedPath{
		...
		{name: "VenvConfigFile", path: layout.VenvConfigFile()},
		...
		{name: "RelayStagingDir", path: layout.RelayStagingDir()},
