...
| State `T/J` create-exclusive leaf | `FILE_READ_DATA \| FILE_WRITE_DATA \| FILE_READ_ATTRIBUTES \| DELETE \| SYNCHRONIZE` | `FILE_SHARE_READ` | `CREATE_NEW` | `FILE_FLAG_OPEN_REPARSE_POINT` | `false` |
| State `D/O/B` strong leaf | `FILE_READ_DATA \| FILE_READ_ATTRIBUTES \| DELETE \| SYNCHRONIZE` | `FILE_SHARE_READ` | `OPEN_EXISTING` | `FILE_FLAG_OPEN_REPARSE_POINT` | `false` |
| State sealed `I` read/unlink leaf | `FILE_READ_DATA \| FILE_READ_ATTRIBUTES \| DELETE \| SYNCHRONIZE` | `FILE_SHARE_READ` | `OPEN_EXISTING` | `FILE_FLAG_OPEN_REPARSE_POINT` | `false` |
...
| `TestStateFiles_FinalizeDeletesBackupBeforeIntent` | commit 后先 unlink `B` 并证明 absent，才 unlink `I` |
...
1. 随机 create-exclusive ordinary leaf，options 含 `FILE_FLAG_DELETE_ON_CLOSE`；
2. 写固定 marker、flush、记录 handle identity；
3. 在 original handle 仍打开时，以精确 FileDispositionInfoEx/两 flags unlink；
4. 从 pinned state-root relative reopen 必须 name-not-found；
5. original handle 仍能读到同 marker 且 file ID 不变；
6. 关闭 handle，再次 relative reopen 必须 name-not-found。
...
`unlinkStateLeaf` 只接受已经从 pinned parent 相对打开并在同一 handle 上证明 ordinary、
non-reparse、single-link、same-volume、预期 leaf/file ID/size/digest 的对象。它用
`setStateDisposition(handle, stateDispositionSpec{FileDispositionInfoEx, exactTwoFlags})`，
成功后在 original handle 仍打开时 relative 复验 name absent；不关闭后按路径重开删除。
...
8. 保持 T handle 执行 `T -> D` flags `0`；只有成功才冻结
   `MutationApplied=true`；
9. commit 后用 O handle POSIX unlink B，在 original/old-reader handle 仍打开时证明 B
   name absent；再次确认 B absent 后才用 intent handle unlink I；
10. 尝试关闭全部 T/J/I/O/N/probe handles，最后关闭 G。
...
| 固定测试 | 核心断言 |
| --- | --- |
| `TestStateFiles_POSIXUnlinkCapabilityAndExactFlags` | 复用已有 WriteAtomic 覆盖，并新增 conditional remove；精确 probe/actual unlink class、flags 与 name-absent 语义 |
...
- probe original handle marker/file ID 改变，或 close/delete-on-close 后 name 仍存在；
- probe 已缓存 success 后 actual unlink 语义漂移；
- B/I/T/J/D 的 actual unlink，且原 leaf/old-reader handle 尚开时 name 已 absent。
...
6. 重验同一 D handle 后，以 exact POSIX flags unlink；成功且 relative name absent 是
   唯一 remove commit；
...
| D unlink 返回或 name-absent 复验不明确 | false | true | 失败关闭，不重试猜测 |
| D POSIX unlink success，所有 closes success | true | false | nil |
| D unlink success，后续复验/finalize/leaf/G close failure | true | true | post-commit/close 链；不得反转 missing |
...
- D unlink success、name absent、original handle 尚未 close 时的 leaf/G close error，
  断言 `{true,true}`；
...
`TestStateFiles_RemoveCancellationAfterDispositionKeepsApplied` 在 D 的
FileDispositionInfoEx 成功且 relative name-absent 已证、original D handle 尚未关闭的
hook 上暂停，再取消 ctx。clean close 行返回 `{true,false}, nil`；注入 leaf/G close
故障行返回 `{true,true}` 和完整链。取消不得覆盖 applied、重新 probe/unlink 或把 stable
missing 反转为存在。两个取消测试只用 channel/barrier，不用 `time.Sleep`。
...
| existing-D 正向 | `T` flush、`J -> I`、`D -> B`、`T -> D`、B unlink、I unlink 前/后 | `T -> D` 前按 sealed intent 返回 old，成功后 new；B/I 成功后 name immediate absent |
...
| conditional remove | D POSIX unlink **调用前**；调用成功、relative name 已 absent、original D handle 仍打开后 | 前者 crash 后 old；后者 crash 后 stable missing，下一 remove 零 probe `{false,false}` |

“确认成功后” cutpoint 必须位于 Win32 返回、relative name absent 已证且原 leaf handle
仍存活的位置。conditional remove 两个 cutpoint 都是真实 child process。success remove
...
- probe 与 actual unlink 都恰为 `FileDispositionInfoEx` +
  `FILE_DISPOSITION_FLAG_DELETE | FILE_DISPOSITION_FLAG_POSIX_SEMANTICS`；原/old-reader
  handle 尚存活时 relative name 已 absent，且无 classic/DELETE-only/path fallback；
...
- `TestWindows_StateGuardReleasedAtEveryCrashCutpoint` 真实 child-process 覆盖 conditional
  remove 的 D unlink 调用前与成功/name-absent/original-handle-open 后，以及全部
