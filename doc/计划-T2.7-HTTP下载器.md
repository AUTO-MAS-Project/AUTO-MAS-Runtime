...
1. 本任务新增的 `downloader.go`、`downloader_client.go` 不导入 `uv`、`gitrepo`、
   `protocol` 或 `logging`，不写 stdout，不选源、不重试、不映射任何 `UV_*`/`GIT_*`
   协议码。`internal/mirror` 的既有 T2.6 `report.go`、`rotation.go` 允许继续导入
   `protocol` 以承载已冻结的 `NETWORK_UNAVAILABLE` / `MIRROR_EXHAUSTED` 错误码；
   T2.7 不得扩大该依赖。
...
Task 3 已冻结行为的 Task 4.3 验收测试、Task 4.2 完整 `abortFailure` 的 Task 4.5 清理验收、
Task 4 的跨包/并发/TLS 测试与 Task 5 组件测试只提供验收证据，不引入新生产行为，因此
直接 GREEN，不伪造 RED。所有微步骤只在所属内部任务末尾统一暂存并提交一次。
...
- [ ] 4.5 落盘三个 `TestDownloader_Cleanup*`；Task 4.2 的最终 `abortFailure` 已使用独立
  cleanup ctx、解释 Abort result 并保存主/cleanup 双链，因此本步骤直接 GREEN 验收：
  确认 operation cancel 不传播而 value 保留、cleanup deadline 有界且清理错误不覆盖主失败。
  若任一测试失败，先按实际失败原因修复，不得回退 Task 4.2 制造 `CleanupErr = nil` 红灯。
...
- [ ] 6.1 先落盘 `TestDownloaderClient_RedirectErrorResponseBodyClosedExactlyOnce`，使用真实
  `http.Client` + 自定义 `RoundTripper` 触发 `CheckRedirect` downgrade 拒绝，取得 Body
  Close count = 2 的精确 RED；不得用直接返回 `response + error` 的 fake 绕过
  `http.Client.Do` 已关闭 Body 的契约。
- [ ] 最小修复只在 `result.err == nil` 时关闭失败分支的 response；`result.err != nil` 时遵循
  `http.Client.Do` 契约，不再关闭已由 Client 关闭的 redirect response Body。
- [ ] 用新测试、既有 redirect/Do-join 测试、mirror 全包、`-count=100`、race 与任务 6
  全部最终门 GREEN；独立复审必须回到零 Critical/Important。
- [ ] 修复仅提交 `downloader_client.go`、`downloader_client_test.go`，提交信息固定为
  `fix: 避免重复关闭重定向响应体 (T2.7)`。
...
普通 Junction 由 PowerShell 7 `New-Item -ItemType Junction` 在普通用户权限下创建；测试路径
通过两个测试专用环境变量传入，因为 `pwsh -Command <string>` 会把后续 argv 解释为命令文本，
不会填充 `$args`。不得改用实验性的 `-CommandWithArgs`，也不得拼接或插值路径到脚本文本。
...
$boundaryScan = @(
    & $rg -n `
        'AUTO-MAS-Runtime/internal/(uv|gitrepo|logging)|os\.Stdout|fmt\.F?Print|log\.Print|json\.NewEncoder' `
        --glob '*.go' --glob '!*_test.go' "internal/mirror" 2>&1
)
...
$downloaderProtocol = @(
    & $rg -n `
        'AUTO-MAS-Runtime/internal/protocol' `
        "internal/mirror/downloader.go" `
        "internal/mirror/downloader_client.go" 2>&1
)
$downloaderProtocolExit = $LASTEXITCODE
$downloaderProtocol
if ($downloaderProtocolExit -gt 1) { throw "downloader protocol scan failed" }
if ($downloaderProtocolExit -eq 0) { throw "downloader imports protocol" }
...
$forbidden = @(
    & $rg -n `
        'AUTO-MAS-Runtime/internal/(uv|gitrepo|logging)|os\.Stdout|os\.Rename|MoveFileExW|os\.Remove(All)?|fmt\.F?Print|log\.Print|json\.NewEncoder' `
        --glob '*.go' --glob '!*_test.go' "internal/mirror" 2>&1
)
...
| Downloader 不选源、不重试、不映射 uv/Git/protocol | 最终包边界扫描 + Downloader 文件 protocol import 扫描 |
...
### 任务 6.1：审查修复——redirect error Body 唯一关闭

**文件：**

- 修改：`internal/mirror/downloader_client.go`
- 修改：`internal/mirror/downloader_client_test.go`

`http.Client.Do` 只会在 `CheckRedirect` 失败时同时返回非 nil response 与 error，并保证
该 response Body 已关闭。`doRequest` 必须把 `result.err != nil` 视为 Client 已完成 Body
收口；只有 `result.err == nil` 但由于 operation cancel/connect timeout 已冻结失败分类时，
才由 Downloader 关闭随成功 Do 结果返回的 response。测试必须通过真实 `http.Client`
触发这个标准库契约，不能让 fake 直接伪造 `response + error`。

- [ ] **步骤 1：落盘固定测试并取得精确 RED**

在 `downloader_client_test.go` 增加：
...
```powershell
& $gofmt -w "internal/mirror/downloader_client_test.go"
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 test gofmt failed" }
Invoke-ExpectedFailure `
    "./internal/mirror" `
    '^TestDownloaderClient_RedirectErrorResponseBodyClosedExactlyOnce$' `
    'Body\.Close count = 2, want 1'
```

预期：测试实际运行并因 `Body.Close count = 2, want 1` 失败；不得接受编译失败、测试未运行
或其他失败原因。

- [ ] **步骤 2：最小修复并验证**

把 `doRequest` 的 `kind != ""` 分支改为：
...
& $gofmt -w `
    "internal/mirror/downloader_client.go" `
    "internal/mirror/downloader_client_test.go"
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 gofmt failed" }
Invoke-PassingTest `
    "./internal/mirror" `
    '^TestDownloaderClient_(RedirectErrorResponseBodyClosedExactlyOnce|RejectsRedirectDowngradeBeforeRequest|AllowsHTTPSRedirectAndLimitsTen|DoResultChannelJoinsAfterCancelOrTimeout)$'
Invoke-PassingTest `
    "./internal/mirror" `
    '^TestDownloaderClient_RedirectErrorResponseBodyClosedExactlyOnce$' `
    -Count 100
& $go test ./internal/mirror -count=1
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 mirror tests failed" }
& git diff --check
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 diff check failed" }
```

- [ ] **步骤 3：独立复审并提交修复**

规格与质量审查确认：redirect error response 只由 `http.Client` 关闭一次；`result.err == nil`
的 operation cancel/connect timeout 成功 response 仍由 Downloader 关闭；错误分类、脱敏、
Do goroutine join 与原始 cause 链不变。Critical/Important 必须为零。

```powershell
& git add -- `
    "internal/mirror/downloader_client.go" `
    "internal/mirror/downloader_client_test.go"
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 staging failed" }
$staged = @(& git -c core.quotePath=false diff --cached --name-only)
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 staged file audit failed" }
$expected = @(
    "internal/mirror/downloader_client.go",
    "internal/mirror/downloader_client_test.go"
)
if ((Compare-Object $expected $staged).Count -ne 0) {
	$staged
	throw "Task 6.1 staged unexpected files"
}
& git commit -m "fix: 避免重复关闭重定向响应体 (T2.7)"
if ($LASTEXITCODE -ne 0) { throw "Task 6.1 commit failed" }
```

提交后从任务 6 步骤 1 开始完整重跑定向、repeat、标准门、全仓 race、静态扫描与最终
独立审查；不得直接进入任务 7。
