...
foreach ($signature in @(
    ...
    'func \(f \*DownloadFiles\) Begin\(ctx context\.Context, name string\) \(\*DownloadSession, error\)',
    'func \(s \*DownloadSession\) Write\(p \[\]byte\) \(int, error\)',
    'func \(s \*DownloadSession\) Path\(\) string',
    'func \(s \*DownloadSession\) PartPath\(\) string',
    'func \(s \*DownloadSession\) PublishNoReplace\(ctx context\.Context\) \(PublishResult, error\)',
...
性粘贴步骤。执行者必须按本节逐项勾选；每项都是一个 2～5 分钟行为单元。任务 1～4 的
生产行为严格执行“落盘一个固定测试 → 取得指定 RED → 落盘最小实现 → 取得 GREEN”；
共享函数按微步骤增量编辑，不得提前复制后续分支。任务 4 的跨包/并发/TLS 测试与任务 5
组件测试只提供验收证据，不引入新生产行为，因此直接 GREEN，不伪造 RED。所有微步骤只在
所属内部任务末尾统一暂存并提交一次。
...
- [ ] 4.2 落盘 `TestDownloader_DownloadFaultMatrix`；确认成功骨架以 `Abort calls = 0` RED；
  ...
  用完整测试名 GREEN。
- [ ] 4.3 落盘 `TestDownloader_BodyCloseFailureMatrix`；确认现有顺序以
  `want checksum_mismatch` RED；最小修正 EOF 内 verify 先于唯一 Close/join，并把预检
  close cause 加入既有主链；用完整测试名 GREEN。
...
任务 5 只有直接 GREEN 验收。每个内部任务末尾的 commit block 是该任务唯一提交点。
