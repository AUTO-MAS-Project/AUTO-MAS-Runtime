...
- `fakeuv`：按 JSON 配置回放 uv 参数、环境、stdout/stderr、退出码和有限副作用；动作中的
  `exec` 与 `execArgs` 可让夹具在保持继承环境和标准流的前提下启动真实测试子进程，用于
  Windows Job/进程树验收；`pidFile` 可原子记录 uv 根 PID，`execReadyFile`/`execReleaseFile`
